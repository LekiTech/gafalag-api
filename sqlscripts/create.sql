...
-- This index is needed to be able to execute the following query: WHERE to_tsvector('simple', ex."raw") @@ to_tsquery('simple', :exp || ':*')
CREATE INDEX example_raw_gin_idx ON example USING GIN (to_tsvector('simple', raw));

-- This index is needed to be able to execute the following query: WHERE ex."raw" ILIKE '%' || :exp || '%'
